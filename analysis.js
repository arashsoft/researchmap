...
exports.award_relationship_extractor =  function award_relationship_extractor(proposal_ID, keyword_filter_array, name_filter_array, begin_date, end_date, threshold, kernel_selection, algorithm_selection, myfunction) {
  ...
  analyzed_award._error = 0;
	...
	function(callback) {
		...
		//check award status
		function(callback) {
			var query_text = "SELECT `ID`, `Proposal`, `AwardStatus` FROM `award` WHERE TRIM(LEADING '0' FROM Proposal) = ".concat(proposal_ID);
      //console.log(query_text);
			connection.query(query_text, function(err, result) {
        if(err) {
            // console.log(query_text);
          ...
          var temp = new Object();
        	...
        	if(_.size(result) == 0) {
          	...
          	analyzed_award._awardStatus = "Unknown";
						analyzed_award._error = 1;
						analyzed_award._note = "Cannot not find the award";
						connection.end();
						myfunction(analyzed_award);
        	}
       ...
       callback();
        ...
        connection.query(query_text, function(err, result) {
            ...
            callback();
				...
				analyzed_award._error = 1;
        analyzed_award._note = "No award granted to this proposal.";
				connection.end();
        myfunction(analyzed_award);
			}
		},
		...
		function(callback) {
          ...
          analyzed_award._error = 1;
          analyzed_award._note = "No investigators associated with this proposal in the database.";
					connection.end();
          myfunction(analyzed_award);
				}
          ...
          analyzed_award._error = 1;
          analyzed_award._note = "No awards were granted to this proposal.";
					connection.end();
          myfunction(analyzed_award);
        }
			}
      ...
      if(_.size(analyzed_award._relatedPublicationsList) > 0) {
		...
		function(callback) {
          ...
          analyzed_award._error = 1;
          analyzed_award._note = "Wrong entry for the aggregation!!!";
					connection.end();
          myfunction(analyzed_award);
				}
			});
      ...
      callback();
		...
		function(callback) {
          ...
          analyzed_award._error = 1;
          analyzed_award._note = "Wrong entry for the algorithm!!!";
					connection.end();
          myfunction(analyzed_award);
				}
			});
